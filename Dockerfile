...
RUN /bin/bash -l -c "service mysqld start && service elasticsearch start && su - zammad && export RAILS_ENV=production && cd /opt/zammad && export PATH=/opt/zammad/bin:$PATH && export GEM_PATH=/opt/zammad/vendor/bundle/ruby/2.2.0/ && rails r \"Setting.set('es_url', 'http://localhost:9200')\" && sleep 15 && rake searchindex:rebuild"
